...
def extract_label(args, model, videoPath, device, savePath):
    ...
    while ret:  # 如果视频仍然存在，继续创建图像
        if frameNum % timeF == 0:
            ...
            output_softmax = softmax(output_softmax)


            max_value = round(max(output_softmax[0].cpu().numpy()),2)

            _label = np.argmax(output_softmax[0].cpu().numpy())
            label.append(_label)

        frameNum += 1

        videoWrite.write(cv2.putText(frame, classes[label[-1]] + ' ' + str(max_value), (200, 200), cv2.FONT_HERSHEY_COMPLEX, 2.0, (100,149,237), 2))
